...
- [x] **Шаг 2:** Проверить, что переключение `isAiMode` мгновенно вызывает `onVote` with новым значением (т.к. `suggestedSpValue` меняется через `useMemo`).
...
- [x] UC2: Выбор в таблице -> Карта сверху выбралась.
- [x] UC1: Клик на карту -> Таблица очистилась.
- [x] UC4: Снятие всех галок в таблице -> Карта сверху погасла.
- [x] UC6: Повторный клик на карту -> Карта погасла, таблица очистилась.
- [x] UC5: Переключение AI под нагрузкой -> Карта меняется мгновенно.
- [x] UC7: Нажатие "Delete estimates" модератором -> Всё чисто у всех.
- [x] UC3: Изменение выбора в таблице -> Подсветка карт переключается.
